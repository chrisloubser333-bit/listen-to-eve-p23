...
class _SettingsScreenState extends State<SettingsScreen> {
        ...
        children: [
          ...
          _glassCard(
            child: Padding(
              ...
              child: Row(children: [
                _choiceChip('xAI (Grok)', settings.aiProvider == 'xai',
                    () => settings.setAiProvider('xai')),
                const SizedBox(width: 10),
                _choiceChip('Google Gemini', settings.aiProvider == 'gemini',
          ...
          _glassCard(
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 4),
              child: Row(children: [
                _choiceChip('English', settings.language == 'en',
                    () => settings.setLanguage('en')),
                const SizedBox(width: 10),
                _choiceChip('Afrikaans', settings.language == 'af',
              ...
              child: _glassCard(
                ...
                child: ListTile(
                  ...
                  leading: Container(
                    ...
                    child: ClipOval(
                      child: Image.asset(
                        ...
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Icon(character.icon,
                            size: 20, color: selected ? _primary : _muted),
                      ),
                    ),
                  ),
                  title: Text(
                  ...
                  subtitle: Padding(
                    ...
                    child: Text(character.getDescription(isAf),
                        style: TextStyle(color: _muted, fontSize: 12)),
                  ),
                  trailing: selected
          ...
          _sectionTitle(
              isAf ? 'Steminteraksie & Pouse' : 'Speech & Silence Detection'),
          _glassCard(
            child: Padding(
          ...
          _glassCard(
            child: ListTile(
              ...
              title: Text(isAf ? 'Bestuur geheue' : 'Manage memory',
                  style: TextStyle(color: _text)),
              subtitle: Text(
                isAf
          ...
          _glassCard(
            child: SwitchListTile(
              secondary: Icon(
                  ...
                  color: _primary),
              title: Text(isAf ? 'Donker tema' : 'Dark theme',
                  style: TextStyle(color: _text)),
              subtitle: Text(
                settings.darkMode
          ...
          const SizedBox(height: 28),
          _sectionTitle(isAf
              ? 'Bediener-URL (Hoof-eindpunt)'
              : 'Server Base URL (Primary Endpoint)'),
          _endpointCard(
            ...
            onSave: () async {
              final messenger = ScaffoldMessenger.of(context);
              await settings
                  .setServerBaseUrl(_serverBaseUrlController.text.trim());
              if (mounted)
                messenger.showSnackBar(SnackBar(
                    content: Text(isAf
          ...
          const SizedBox(height: 28),
          _sectionTitle(isAf
              ? 'Spesifieke websoek-eindpunt (Opsioneel)'
              : 'Custom Web Search Endpoint (Optional)'),
          _endpointCard(
            ...
            onSave: () async {
              final messenger = ScaffoldMessenger.of(context);
              await settings
                  .setSearchProxyEndpoint(_proxyController.text.trim());
              if (mounted)
                messenger.showSnackBar(SnackBar(
                    content: Text(isAf
          ...
          const SizedBox(height: 28),
          _sectionTitle(isAf
              ? 'Spesifieke prentgenerering-eindpunt (Opsioneel)'
              : 'Custom Image Generation Endpoint (Optional)'),
          _endpointCard(
            ...
            onSave: () async {
              final messenger = ScaffoldMessenger.of(context);
              await settings
                  .setImageProxyEndpoint(_imageProxyController.text.trim());
              if (mounted)
                messenger.showSnackBar(SnackBar(
                    content: Text(isAf
          ...
          OutlinedButton.icon(
            style: OutlinedButton.styleFrom(
                ...
                side: BorderSide(color: _scheme.error)),
            onPressed: () async {
                  ...
                  actions: [
                    ...
                    FilledButton(
                      style: FilledButton.styleFrom(
                          ...
                          foregroundColor: _scheme.onError),
            ...
            label:
                Text(isAf ? 'Vee kletsgeskiedenis uit' : 'Clear chat history'),
          ),
        ],
      ),
    );
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: selected
            ? [
                BoxShadow(
                    color: _primary.withValues(alpha: 0.18), blurRadius: 12)
              ]
            : null,
      ),
      child: child,
  ...
  Widget _endpointCard({
    ...
    if (!initialized && controller.text != initialValue)
