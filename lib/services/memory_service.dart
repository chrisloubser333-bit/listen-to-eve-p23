...
class MemoryService extends ChangeNotifier {
  ...
  Future<List<MemoryItem>> retrieveRelevantMemories(
    ...
    const stopWords = {
      ...
      'die', 'wat', 'op', "'n", 'en', 'na',
      'vir', 'van', 'oor', 'deur', 'dat', 'hierdie', 'dit', 'daardie',
      // Afrikaans pronouns
    ...
    final isAboutMeQuery = queryClean.contains('who am i') ||
        ...
        queryClean.contains('about me') ||
        queryClean.contains('about myself') ||
        queryClean.contains('oor my') ||
        queryClean.contains('oor myself') ||
        queryClean.contains('facts about me') ||
        queryClean.contains('find about me') ||
        queryClean.contains('know about me') ||
        queryClean.contains('wat weet jy van my') ||
        queryClean.contains('wat weet jy oor my') ||
        queryClean.contains('wat kan jy oor my');

    if (tokens.isEmpty && !isAboutMeQuery) return [];
    ...
    for (final memory in candidatePool) {
      ...
      // Identity & personal facts query match
      if (isAboutMeQuery &&
          (memory.topicKey == 'user_name' ||
              memory.topicKey == 'user_surname' ||
              memory.category == MemoryCategory.semantic ||
