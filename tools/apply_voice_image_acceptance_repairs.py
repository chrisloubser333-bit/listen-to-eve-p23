...
# JustAudio begins play, or from flutter_tts's native start handler.
...
if "Function()? _pendingPlaybackStarted;" not in text:
    ...
    print("voice_service.dart already contains playback-start support")
...
# speech cannot start. Accept either implementation from the earlier/manual
# repair so a resumed run does not abort before image save/share is repaired.
...
new = """      final assistantMsg = ChatMessage(
      ...
      // Reveal it at playback start. If voice fails before playback starts,
      // reveal after speak() returns so text is never lost.
      if (_settings.autoVoiceReply && finalReply.trim().isNotEmpty) {
        var revealed = false;
        void revealReply() {
          if (revealed) return;
...
elif "onPlaybackStarted:" in text:
    print("chat_provider.dart already contains playback-start reveal support")
else:
...
if start == -1 or end == -1:
    if "Future<Uint8List> _resolveImageBytes()" in text and "Widget _buildImageContent()" in text:
        print("exploded_image_modal.dart already contains native save/share support")
    else:
        raise SystemExit("ABORT: image save/share handlers not found; restore the pre-final-acceptance modal backup and rerun")
else:
  ...
  Future<void> _handleSave(BuildContext context) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isAvatar ? "$title's portrait was saved to your gallery." : 'Image saved to your gallery.'),
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Could not save image: $e'), behavior: SnackBarBehavior.floating),
      );
    }
  }
  ...
  Future<void> _handleShare(BuildContext context) async {
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Could not share image: $e'), behavior: SnackBarBehavior.floating),
      );
    }
  }
